...
//    private static Logger logger = Logger.getLogger(DateUtil.class);
    ...
    public static Date addMinute(Date nowDate, int period) {
        LocalDateTime localDate = new LocalDateTime(nowDate);
        return localDate.plusMinutes(period).toDate();
    }
     ...
     * @return Date
     */
    public static Date addHour(Date nowDate, int period) {
        LocalDateTime localDate = new LocalDateTime(nowDate);
        return localDate.plusHours(period).toDate();
    }

    public static Date addSecond(Date nowDate, int period) {
        LocalDateTime localDate = new LocalDateTime(nowDate);
        return localDate.plusSeconds(period).toDate();
    }
...
//            logger.error(ex.getMessage(), ex);
    ...
    public static Date safeStringToDateByPattern(String dateStr) {
        String pattern = Const.DATE_FORMAT;
        try {
...
////        now2 = DateUtil.addHour(now2, -2);
////        now2 = DateUtil.addDay(now, -2);
//        now2 = getFirstDayOfMonth(now2);
//
//        now2 = string2Date("01/05/2017");
//        now = string2Date("04/06/2017");
//
//        System.out.println(addHour(now, -1));
//
//        System.out.println(DateUtil.date2ddMMyyyyHHMMss(now));
//        System.out.println(DateUtil.date2ddMMyyyyHHMMss(now2));
//
//        System.out.println("getDay2=" + getDate(now2));
//
//        int sss = monthsBetweenRounding(now2, now);
//        System.out.println("monthsBetween=" + sss);
//
//        int dateSta = DateUtil.getDate(now);
//        int dateSys = DateUtil.getDate(now2);
//        if (dateSta >= dateSys) {
//            sss = sss + 1;
...
//            logger.error(e.getMessage(), e);
    ...
    /**
     * Tinh nam tron xuong: 19/07/2002 -> 20/07/2017 thi la 15 nam, 1 ngay -> tra ve 15
     *
     * @param date1
     * @param date2
     * @param exactToDate
     * @return
     */
    public static int yearsBetween(Date date1, Date date2, boolean exactToDate) {
        if (exactToDate) {
            return Years.yearsBetween(new DateTime(date1), new DateTime(date2)).getYears() + 1;
        } else {
            return new DateTime(date2).getYear() - new DateTime(date1).getYear();
        }
    }

    /**
     * Tinh nam tron len: 19/07/2002 -> 20/07/2017 thi la 15 nam, 1 ngay -> tra ve 16
     *
     * @param date1
     * @param date2
     * @param exactToDate
     * @return
     */
    public static int yearsBetweenUpper(Date date1, Date date2, boolean exactToDate) {
        if (exactToDate) {
            return Years.yearsBetween(new DateTime(date1), new DateTime(date2)).getYears() + 1;
        } else {
            return new DateTime(date2).getYear() - new DateTime(date1).getYear();
    ...
    /**
     * ham lay thang chenh lech, lam tron thang
     * input date 1 < date 2
     *
     * @param date1
     * @param date2
     * @return
     */
    public static int monthsBetweenRounding(Date date1, Date date2) {
        int result = 0;
        date1 = getFirstDayOfMonth(date1);
        date2 = getFirstDayOfMonth(date2);
        while (true) {
            int compare = compareDateToDateTime(date1, date2);
            if (compare < 0) {
                result++;
            } else {
                result++;
    ...
    public static Date addYear(Date date, int numYear) {
        return new LocalDateTime(date).plusYears(numYear).toDate();
    }
    ...
    /**
     * Ham compare 2 ngay den gio phut giay
     *
     * @param date1
     * @param date2
     * @return < 0 neu date 1 < date2 , = 0 neu date 1 = date 2, > 0 neu date 1 > date 2
     */
...
//            logger.error(ex.getMessage(), ex);
    ...
    /**
     * Tra ve thoi diem ket thuc ngay 23:59:59:999
     *
     * @param date
     * @return
     */
    ...
    /**
     * Kiem tra 2 ngay co nam cung 1 thang hay khong
     *
     * @param date1
     * @param date2
     * @return
     */
    public static boolean checkTwoDateSameMonth(Date date1, Date date2) {
        if (date1 == null || date2 == null) {
            return false;
        }
        DateTime dt1 = new DateTime(date1);
        DateTime dt2 = new DateTime(date2);
        return dt1.getMonthOfYear() == dt2.getMonthOfYear() && dt1.getYear() == dt2.getYear();
    }
    ...
    public static boolean isDuplicateDateTime(Date effectFrom, Date until, Date oldEffectFrom, Date oldUntil) {
        ...
        if (oldUntil == null) {
            ...
            if (until == null) {
                ...
                if (until.equals(oldEffectFrom) || until.after(oldEffectFrom)) {
            ...
            if (until == null) {
                //neu den ngay moi == null
                if (effectFrom.equals(oldUntil) || effectFrom.before(oldUntil)) {
                ...
                if (effectFrom.before(oldEffectFrom) && until.after(oldUntil)) {
                    return true;
                }
            }
        }
...
//                        oldValue = DateTimeUtils.convertDateTimeToString((Date) oldBO);
    ...
    public static boolean isValidDate(String dateToValidate) {
        ...
        return isValidFormatDate(dateToValidate, Const.DATE_FORMAT);
    }
...
//            logger.error(e.getMessage(), e);
    ...
    public static int getDayBetween2DateString(String firstdate, String seconddate) {
        try {
...
//            logger.error(ex.getMessage(),ex);
    ...
    public static int getDayBetween2DateString(String firstdate, String seconddate, String pattern) {
        try {
            DateTimeFormatter dateStringFormat = DateTimeFormat
                    .forPattern(pattern);
            DateTime firstTime = dateStringFormat.parseDateTime(firstdate);
